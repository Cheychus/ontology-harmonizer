<script lang="ts">
    ...
    import CollectionComponent from "../terminology/CollectionComponent.svelte";
...
<div class="flex w-full flex-col gap-2 py-4">
    ...
    <div class="flex gap-4" role="status" onmouseenter={() => preloadData("/collections")}>
        <h3>Collection</h3>
        <Button
            variant={"outline"}
            size={"icon"}
    ...
    {#if terminologyStore.selectedCollection}
        <CollectionComponent collection={terminologyStore.selectedCollection} selectButton={false} />
    {/if}
